...
function get_event(res, sport){
		query="SELECT distinct Event From Athlete_won where Sport='" + sport +"' ;";
...
function get_season(res, edition){
		query="SELECT distinct Season From Medal_Tally where Year=" + edition +" ;";
		connection.query(query, function(err, rows, fields) {
			if (err) {
				console.log(err);	
					 }
...
function get_sport(res, edition, season ){
		query="SELECT distinct Sport From Athlete_won where Year=" + edition +" and Season='" + season + "';";
		connection.query(query, function(err, rows, fields) {
			if (err) {
				console.log(err);	
					 }
...
function init_query_db(res) {
	query1 = "SELECT distinct Year FROM Medal_Tally;";
	query2 = "SELECT distinct Season FROM Medal_Tally;";
	query3 = "SELECT distinct Sport FROM Athlete_won;";
	query4 = "SELECT distinct Event FROM Athlete_won;";
